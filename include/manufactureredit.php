...
<div style="border-bottom: 1px #CCC solid; font-size: 20px;margin-bottom: 30px;"><?php echo $rLanguage->text("Update Manufacturer");?></div>
<div class="CAR_REGISTRATION" style="height: 500px;">
	<form action="<?php echo HTTP_DOMAIN;?>include/php_sub/r_manufactureredit.php" id="iform_manufactureredit" method="post" enctype="multipart/form-data">
    	<table border="0" width="600px">
        	<tr>
            	<td class="label"><?php echo $rLanguage->text("Manufacturer");?></td>
                <td>
    				<input type="hidden" class="textbox" name="manufacturerid" id="imanufacturerid" value="<?php echo $manufacturer_id; ?>"/>           
	                <input type="text" class="textbox" name="manufacturer" id="imanufacturer" value="<?php echo $manufacturer->getManufacturerName();?>" />	
                </td>
            </tr>
            <tr>
            ...
            <tr>
            	<td class="label"></td>
                <td>
                	<input type="button" name="btn_edit" id="ibtn_edit" value="<?php echo $rLanguage->text("Update");?>" onclick="submitManufacturerEdit();" />
...
<script type="text/javascript">
	function submitManufacturerEdit(){
		if(file_validation('imanufacturericon')==true){
			$('form#iform_manufactureredit').submit();
		}
	}
	function goBack(){
		 history.back();
